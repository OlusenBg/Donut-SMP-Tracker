        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          {items.map((item) => (
            <Link
              key={item.slug}
              href={`/item/${item.slug}`}
              className="flex flex-col items-center rounded-2xl border border-donut-500/20 bg-donut-900/50 p-4 transition-all hover:-translate-y-1 hover:border-donut-accent/60 hover:shadow-glow"
            >
              <div className="flex h-16 w-16 items-center justify-center rounded-xl bg-donut-800/60 p-2.5">
                <Image
                  src={item.image}
                  alt={item.name}
                  width={52}
