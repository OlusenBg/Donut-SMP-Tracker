    ...
    async function load() {
      try {
        const res = await fetch(
          `/api/items?category=${encodeURIComponent(category)}&exclude=${encodeURIComponent(excludeSlug)}&limit=${LIMIT}`,
        );
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              key={item.slug}
              href={`/item/${item.slug}`}
              className="relative flex w-32 flex-shrink-0 flex-col items-center rounded-2xl border border-donut-500/20 bg-donut-900/40 p-3 transition-colors hover:border-donut-accent/60 hover:shadow-glow"
            >
              ...
              <div className="mt-4 flex h-14 w-14 items-center justify-center rounded-lg bg-donut-800/60 p-2">
                <Image
                  src={item.image}
                  alt={item.name}
                  width={44}
